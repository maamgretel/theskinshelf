...
// Open Edit Product Modal - FIXED VERSION
async function openEditProductModal(productId) {
  ...
  try {
    ...
    // Populate form with product data - ENSURE ALL FIELDS ARE POPULATED
...
async function handleEditProduct() {
  ...
  try {
    ...
    console.log(`[DEBUG] Updating product ${currentEditProductId}`);
...
async function handleAddStock() {
  ...
  try {
    // First, try the dedicated add-stock endpoint
    const payload = {
    ...
    if (!response.ok) {
      ...
      // Fallback: Update the product directly with new stock value
      const currentStock = parseInt(document.getElementById('modalCurrentStock').value) || 0;
      const newStock = currentStock + stockToAdd;
      
      // Try multiple methods for updating stock
      ...
      for (const method of methods) {
        try {
          ...
          if (note) {
            stockPayload.update_note = `Added ${stockToAdd} items. Note: ${note}`;
          }
          
          console.log(`[DEBUG] Trying ${method} for stock update:`, stockPayload);
          
          response = await window.safeFetch(`${BACKEND_URL}/api/products/${currentProductId}`, {
            method: method,
          ...
          if (response.ok) {
            ...
            console.log(`[DEBUG] ${method} stock update successful:`, result);
            break;
          } else {
            const errorText = await response.text();
            console.log(`[DEBUG] ${method} failed (${response.status}):`, errorText);
          ...
          console.log(`[DEBUG] ${method} error:`, methodError.message);
          continue;
        }
      }
      
      if (!stockUpdateSuccess) {
        throw new Error('Failed to update stock using any available method');
      }
    } else {
      result = await response.json();
      console.log(`[DEBUG] Add stock endpoint successful:`, result);
    }
    
    alert(`Successfully added ${stockToAdd} items to stock. New total: ${result.new_stock || 'Updated'}`);
